...
public class MessagesListAdapter<MESSAGE extends IMessage>
    ...
    public void addToStart(MESSAGE message, boolean scroll) {
        boolean isNewMessageToday = !isPreviousSameDate(0, message.getCreationDate());
        if (isNewMessageToday) {
            items.add(0, new Wrapper<>(message.getCreationDate()));
        }
    ...
    public void addToEnd(List<MESSAGE> messages, boolean reverse) {
        ...
        if (!items.isEmpty()) {
            ...
            if (DateFormatter.isSameDay(messages.get(0).getCreationDate(), lastItem)) {
    ...
    protected void generateDateHeaders(List<MESSAGE> messages) {
        for (int i = 0; i < messages.size(); i++) {
            ...
            if (messages.size() > i + 1) {
                ...
                if (!DateFormatter.isSameDay(message.getCreationDate(), nextMessage.getCreationDate())) {
                    this.items.add(new Wrapper<>(message.getCreationDate()));
                }
            } else {
                this.items.add(new Wrapper<>(message.getCreationDate()));
            }
    ...
    private int getMessagePositionById(int id) {
        for (int i = 0; i < items.size(); i++) {
            ...
            if (wrapper.item instanceof IMessage) {
                ...
                if (String.valueOf(message.getId()).contentEquals(String.valueOf(id))) {
                    return i;
                }
            }
        }
    ...
    private boolean isPreviousSameDate(int position, Date dateToCompare) {
        ...
        if (items.get(position).item instanceof IMessage) {
            Date previousPositionDate = ((MESSAGE) items.get(position).item).getCreationDate();
